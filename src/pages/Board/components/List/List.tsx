...
const List = (list: IList) => {
  ...
  const handleDragStart = async (event: React.DragEvent<HTMLDivElement>, cardId: number, position: number, list_id: number) => {
  ...
  const handleDrop = async (event: React.DragEvent<HTMLDivElement>, targetListId: number) => {
    event.preventDefault();
    ...
    if (droppedCardId && slotIndex !== null) {
      const updatedCards = list.cards
        ...
        .map((card) => ({
          id: card.id,
          position: card.position >= slotIndex ? card.position + 1 : card.position,
          list_id: targetListId,
        }));
  
      const newCard = { id: droppedCardId, position: slotIndex, list_id: targetListId };
          ...
          .map((card, index) => (
            <React.Fragment key={card.id}>
              ...
              <div
                ...
                draggable="true"
                onDragStart={(event) => handleDragStart(event, card.id, card.position, list.id)}
                onDragOver={(event) => handleDragOver(event, index)}
                onDragEnd={handleDragEnd}
                onClick={(event)=>event.preventDefault()}
              >
                <Card  listId={list.id} card={card} boardId={board_id} onCardUpdating={list.onCardCreated} />
              </div>
            </React.Fragment>
          ))}
        {slotIndex === list.cards.length && <div className="slot"></div>}
      </div>
      <CreateNewCard boardId={board_id} listId={list.id} currentCards={list.cards} onCardCreate={list.onCardCreated} />
      <RemoveList boardId={board_id} listId={list.id} onListRemove={list.onCardCreated} />
    </div>
  );
};
